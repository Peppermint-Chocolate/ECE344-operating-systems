...
/* creating a 1 MiB ext2 file system with 1 KiB sized blocks and space for 128 inodes */
/* 4 inodes: the root directory, the lost+found directory, a regular file named hello-world, and a symbolic link named hello which points to hello-world */
...
void write_block_bitmap(int fd) {
	/* This is all you */
    off_t boffset = BLOCK_OFFSET(BLOCK_BITMAP_BLOCKNO); 
    off_t offset = lseek(fd, boffset, SEEK_SET); 
    ...
    for (int i = 0; i <= 1; ++i) {
        bbm[i] = 0xFF; // set bits to 1 
    }

    bbm[2] = 0x7F; 
    bbm[127] = 0x80; 

    for (int i = NUM_BLOCKS / 8; i <= NUM_BLOCKS - 1; ++i) {
        bbm[i] = 0xFF; 
    }

    if (write(fd, bbm, BLOCK_SIZE) != BLOCK_SIZE) {
        errno_exit("write");
    }
...
void write_inode_bitmap(int fd) {
	/* This is all you */
    off_t boffset = BLOCK_OFFSET(INODE_BITMAP_BLOCKNO); 
    off_t offset = lseek(fd, boffset, SEEK_SET); 
    ...
    for (int i = NUM_INODES / 8; i <= NUM_BLOCKS - 1; ++i) {
        ibm[i] = 0xFF; 
    }
	
    if (write(fd, ibm, BLOCK_SIZE) != BLOCK_SIZE) {
        errno_exit("write");
    }
}
...
void write_inode_table(int fd) {
	...
	hw_inode.i_mode = EXT2_S_IRUSR
	                              | EXT2_S_IWUSR
	                              | EXT2_S_IRGRP
	                              | EXT2_S_IROTH
                                  | EXT2_S_IFREG; 
	hw_inode.i_uid = 1000; // uid and gid to 1000 
	hw_inode.i_size = 12; // size to 12 
	...
	hello_inode.i_mode = EXT2_S_IRUSR
	                              | EXT2_S_IWUSR
	                              | EXT2_S_IRGRP
	                              | EXT2_S_IROTH
                                  | EXT2_S_IFLNK; 
	hello_inode.i_uid = 1000; // uid and gid to 1000 
    ...
    strncpy((char *)&hello_inode.i_block, "hello-world", sizeof(hello_inode.i_block));
	write_inode(fd, HELLO_INO, &hello_inode);
}

void write_root_dir_block(int fd) {
	/* This is all you */
    off_t boffset = BLOCK_OFFSET(ROOT_DIR_BLOCKNO); 
    off_t offset = lseek(fd, boffset, SEEK_SET); 
...
void write_hello_world_file_block(int fd) {
	/* This is all you */
    off_t boffset = BLOCK_OFFSET(HELLO_WORLD_FILE_BLOCKNO); 
    off_t offset = lseek(fd, boffset, SEEK_SET); 
...
int main() {
	...
	write_inode_bitmap(fd); // start 1
	write_inode_table(fd); // 128 inodes x 128 bytes per inode 
